...
@dp.callback_query_handler(text='find_pottery', state=None)
...
@dp.callback_query_handler(text=['find_pottery', 'save_pottery', 'reset_pottery'], state="*")
async def find_id(callback: types.CallbackQuery, state: FSMContext):
    await callback.answer()
...
def register_handlers_admin_find(dp_in: Dispatcher):
    ...
    dp_in.register_message_handler(find_info, regexp=r"\d+", state=FindPottery.id)
